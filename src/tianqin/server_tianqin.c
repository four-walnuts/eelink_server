...
static void read_cb(struct bufferevent *bev, void *arg)
{
    ...
    while ((n = bufferevent_read(bev, buf, sizeof(buf))) > 0)
    {
    	...
    	int rc = handle_tianqin_msg(buf, n, arg);
    	if (rc)
    	{
    		LOG_ERROR("handle tianqin message error!");
    	}
    }
}
...
static void event_cb(struct bufferevent *bev, short events, void *arg)
{
	...
	{
		LOG_INFO("tianqin connection timeout!");
	...
	else if (events & (BEV_EVENT_EOF | BEV_EVENT_ERROR))
	{
		...
		LOG_INFO("Closing the tianqin connection");
    ...
    evutil_socket_t fd, struct sockaddr *address, int socklen, void *arg)
{
	...
	LOG_INFO("tianqin connect from %s:%d", addr, ntohs(p->sin_port));
	struct event_base *base = evconnlistener_get_base(listener);
	struct bufferevent *bev = bufferevent_socket_new(base, fd, BEV_OPT_CLOSE_ON_FREE);
	if (!bev)
	{
		LOG_FATAL("accept tianqin connection failed!");
		return;
	}
	...
	if(!session)
	{
	    LOG_FATAL("session memory alloc failed");
	    return;
	}
	session->base = base;
	session->bev = bev;
...
struct evconnlistener* server_tianqin(struct event_base* base, int port)
{
